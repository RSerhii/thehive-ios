...
class ShareToTheHiveViewController: SLComposeServiceViewController {
    ...
    private func urlScheme(for url: String) -> URL? {
        return URL(string: "thehive://open-url?url=https://thehive.com/discussion/begin?url=\(url)")
    }
    ...
    override func configurationItems() -> [Any]! {
        ...
        provider.loadItem(of: provider.isUrl ? kUTTypeURL : kUTTypeText) { item, error in
            ...
            if let thehiveUrl = urlItem.absoluteString.addingPercentEncoding(withAllowedCharacters: .alphanumerics).flatMap(self.urlScheme) {
                self.handleUrl(thehiveUrl)
            }
        }
